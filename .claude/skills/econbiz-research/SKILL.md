...
用户要“帮我判断怎么回归、处理数据并跑出来”时，串起 analysis 的实际数据检查、design 的简短方案及 analysis 的自动编程执行。沿用已有授权，自动完成脚本、运行和技术修错；只集中询问尚未解决的实质研究选择。交付数据、代码、表图、日志与核验状态，不把繁琐编程交还用户。
...
用户要求 LaTeX、模型公式或 PDF 结果时，读 interpret 和 [LaTeX 结果交付](../../../docs/research-handbook/latex-delivery.md)。沿现有单模型或比较记录导出，检查真实编译结果并逐页核对；源码、PDF 与视觉观察分别保存，不能把源码生成写成 PDF 或版式通过。
