...
支持 CSV、指定工作表的 XLSX、描述统计、企业/年度一项或两项固定效应的线性关联分析。标准误支持 classical、hc1、单维 cluster；完整案例、保留单例、95% 区间。默认 Python 正式估计，可显式选择 Stata；两者均用独立 Python 实现复核。DID、IV、权重、多维聚类及自动模型搜索尚未接入。
