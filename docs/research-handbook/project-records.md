...
正式运行包、报告及重跑见 [正式分析](analysis-execution.md)。结果和报告共用版本依赖链；文件快照须经过当前状态与散列检查后再消费。
...
具体可运行示例见 [工具契约](tool-contracts.md)。

## 可选研究 Git

只有用户选择启用时，才用 `enable_git(workspace, tracked_paths)` 在该研究根建立独立仓库，登记明确的代码、文本及元数据范围。先用 `preview_changes(workspace, paths)` 核对文件清单，再沿已有授权调用 `commit_changes(workspace, paths, message)`。
首次提交需要本机已有有效 Git 身份；工具不替用户创建身份。已有暂存内容、原始数据/论文全文范围、隐藏配置、符号链接、非 UTF-8 或超过 1 MiB 的文件会拒绝；删除文件的 Git 提交暂需另行核实。文件名按字面匹配，含方括号等字符也不会扩展到其他文件。
该功能不设置远端、不推送、不修改全局配置。未启用 Git 时，前述材料版本、检查点和恢复仍然可用。仍从工具仓库根加载 Agent 入口；单独打开嵌套研究仓库尚未实测。
